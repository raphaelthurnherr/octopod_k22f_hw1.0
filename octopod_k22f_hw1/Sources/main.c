...
const unsigned char HeartBitMsg[] = "Octopod controller K22F v1.0.0 - 18.09.2015";
...
// SERVOMOTEUR EN POSITION, ENVOIE d'INTERRUPTION, !!!! SERVO NO 25, 26(EYES, spare) non comptabilisé dans cette interruption
		...
		// -----------------------------------------------------------------------------------
		// --- Raffraichissement des servomoteur
		// -----------------------------------------------------------------------------------
		if(TimeSmoothMotorsRefresh==0){

			// Incrementation en mode smooth si consigne pas atteinte
			smoothUpdateMotors();
			ConvertAngleToPCA9685value();

			// Redemarre la séquence de raffraichissement moteur
			if(MotorToUpdate>=26) MotorToUpdate=0;

	  		//	  		PCA9685SetAllPWMvalue();
		...
		if(MotorToUpdate<26){
			PCA968SetOnePWMvalue(MotorToUpdate);
			MotorToUpdate++;
		}
  ...
  void processCommand(void){
	...
	toggleLed=!toggleLed;
  	...
  	case SM_SMOOTH:
  		...
  		sm_smoothIncrement=(9-smoothValue);
